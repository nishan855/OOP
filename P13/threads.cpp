...
class Prime_numbers {
  public:
    ...
    void find_primes(int lower, int upper){
         ...
         int length;
         int i,l_bound,u_bound;
         length=ceil((lower+upper)/(NUM_THREADS*1.0));
    for(i=0;i<NUM_THREADS+1;i++){
        l_bound = lower+i*length;
        u_bound = std::min(lower + (i+1)*length, upper);
        std::thread* thr1= new std:: thread(&Prime_numbers::find_primes_thread,this,l_bound,u_bound);
             thr.pushback(thr1);
        }
        for(auto x:thr){
                 x->join();
